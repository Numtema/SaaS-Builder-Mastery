...
const Badge = ({ children }: { children?: React.ReactNode }) => (
  <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white/5 text-slate-300 border border-white/10 mb-6 backdrop-blur-sm">
    {children}
  </span>
);
...
const Header = () => {
  return (
    ...
    >
      <div className="bg-slate-950/80 backdrop-blur-xl border border-white/5 rounded-full px-6 py-3 flex items-center justify-between w-full max-w-5xl shadow-2xl">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-slate-950 font-bold">
        ...
        <button className="bg-white text-black hover:bg-slate-200 px-5 py-2 rounded-full text-sm font-semibold transition-all hover:scale-105 active:scale-95 flex items-center gap-2">
          Accéder
...
const Hero = () => {
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-32 pb-20 overflow-hidden">
      {/* Background Ambience - Soft & Monochrome */}
      <div className="absolute inset-0 w-full h-full bg-background overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-white/5 rounded-full mix-blend-screen filter blur-[128px] animate-blob" />
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-slate-500/10 rounded-full mix-blend-screen filter blur-[128px] animate-blob animation-delay-2000" />
        <div className="absolute bottom-1/2 left-1/2 w-96 h-96 bg-white/5 rounded-full mix-blend-screen filter blur-[128px] animate-blob animation-delay-4000" />
      ...
      <div className="container mx-auto px-4 relative z-10 text-center">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button className="w-full sm:w-auto px-8 py-4 bg-white text-black rounded-full font-bold text-lg hover:bg-slate-200 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2 shadow-[0_0_20px_-5px_rgba(255,255,255,0.3)]">
              Accéder à la formation
              <Zap className="w-5 h-5 fill-current" />
            </button>
            <button className="w-full sm:w-auto px-8 py-4 bg-transparent border border-white/10 rounded-full text-white font-medium hover:bg-white/5 transition-all backdrop-blur-sm">
              Voir le programme
            </button>
          </div>
        </motion.div>
      ...
      {/* Decorative Floating UI Elements - Monochrome */}
      <motion.div 
        className="absolute bottom-20 left-10 hidden lg:block p-4 rounded-xl bg-slate-950/90 backdrop-blur-md border border-white/10 shadow-2xl"
      ...
      >
        ...
        <div className="font-mono text-xs text-slate-500">
          <span className="text-white">const</span> app = <span className="text-slate-300">buildSaaS</span>();<br/>
          app.<span className="text-slate-300">deploy</span>(<span className="text-slate-400">"stable"</span>);
        </div>
      </motion.div>
    </section>
  );
...
const Marquee = () => {
  return (
    <div className="w-full bg-black border-y border-white/5 py-8 overflow-hidden relative">
      ...
      <div className="flex animate-scroll w-[200%]">
        <div className="flex gap-16 px-8 min-w-full justify-around items-center opacity-40 grayscale hover:opacity-100 transition-all duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {problems.map((p, i) => (
          <motion.div 
            ...
            viewport={{ once: true }}
            className="group p-8 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all duration-300 relative overflow-hidden"
          >
            <AlertTriangle className="w-8 h-8 text-white mb-6 opacity-80 group-hover:opacity-100 transition-opacity" />
    ...
    <section className="py-20 bg-black/40 border-y border-white/5">
      <div className="container mx-auto px-4 flex flex-col md:flex-row items-center gap-12 md:gap-20">
        <div className="md:w-1/2">
          <div className="relative aspect-square md:aspect-auto md:h-[500px] w-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl grayscale">
             <img 
             ...
             <div className="absolute bottom-8 left-8 right-8">
               <div className="bg-slate-900/80 backdrop-blur-md border border-white/10 p-4 rounded-xl flex items-start gap-3">
                 <ShieldAlert className="w-6 h-6 text-white shrink-0 mt-1" />
                 <div>
                    <h4 className="font-bold text-white">System Critical Error</h4>
                    <p className="text-xs text-slate-400 mt-1 font-mono">Uncaught TypeError: Cannot read properties of undefined</p>
                 </div>
               </div>
             </div>
          </div>
        ...
        <div className="md:w-1/2">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-8">
            ...
            C'est un problème de <span className="text-slate-400 underline decoration-white/20 underline-offset-4">structure</span>.
          </h2>
...
const Solution = () => {
  ...
  return (
    <section id="benefices" className="py-32 container mx-auto px-4">
      <div className="bg-white/[0.03] rounded-3xl p-8 md:p-16 border border-white/5 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="md:w-1/2">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-6">
              Retrouvez clarté et <span className="text-slate-400">vitesse d'exécution</span>.
            </h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 max-w-4xl mx-auto">
        {modules.map((mod, i) => (
          <motion.div
            ...
            className="aspect-[4/3] rounded-2xl bg-white/[0.02] border border-white/5 flex flex-col items-center justify-center p-6 text-center hover:bg-white/[0.05] hover:border-white/20 transition-all cursor-default group"
          >
  ...
  return (
    <div className="relative h-[80vh]" ref={footerRef}>
       <div className="sticky bottom-0 w-full h-[80vh] flex flex-col justify-center bg-black overflow-hidden">
          {/* Footer Background Effect - Clean */}
